...
1. **Быстрый контекст**
   → [`project/overview.md`](./project/overview.md) — кратко, чем живёт компания.
   → [`project/company-info.md`](./project/company-info.md) — юридические реквизиты Edelweiss.
   → [`project/glossary.md`](./project/glossary.md) — расшифровка терминов (Pflegegrad, Verordnung, SGB V, Tour, PDL, Fachkraft и т.д.).
2. **Детали интервью**
   → [`docs/reports/00-summary.md`](./docs/reports/00-summary.md).
3. **Структурный анализ**
   → [`project/subjects/`](./project/subjects) — кто (пациент, врач, касса, сотрудник, система).
   → [`project/cases/`](./project/cases) — кейсы и сценарии.
   → [`project/processes/`](./project/processes) — как склеивается end-to-end.
4. **Точки роста**
   → [`project/optimization/`](./project/optimization) — где болит и что можно улучшить.
...
Живой документ. При появлении новых интервью — кладём `.stt.txt` в `docs/`, после обработки — `*.md` в `docs/reports/`, а обобщения — в `project/`.
